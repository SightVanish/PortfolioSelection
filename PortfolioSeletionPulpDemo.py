...
def ReportStatus(msg, flag):
    sql = "update fll_t_dw.biz_fir_query_parameter_definition set python_info_data='{0}', success_flag='{1}' where id='{2}'".format(
        msg, flag, queryID)
    print("============================================================================================================================")
    print("Reporting issue:", msg)
    conn = psycopg2.connect(host="10.18.35.245", port="5432",
                            dbname="iflorensgp", user="fluser", password="13$vHU7e")
...
try:
    ...
    sqlInput = "select billing_status_fz as billing, unit_id_fz, product, fleet_year_fz as fleet_year, contract_cust_id as customer, contract_lease_type as contract, cost, nbv, age_x_ceu as weighted_age, query_id from fll_t_dw.biz_ads_fir_pkg_data WHERE query_id='{0}'".format(
        queryID)
...
try:
    ...
    fleetAgeAvgLimit = param['containersAge']['average']['averageContainersAge'] if param[
        'containersAge']['average']['averageContainersAge'] else None
    ...
    weightedAgeAvgLimit = param['weightedAge']['average']['averageWeighedAge'] if param[
        'weightedAge']['average']['averageWeighedAge'] else None
    ...
    topLesseeLimit = [param['TopLessee'][0]['percent'] / 100 if param['TopLessee'][0]['Top1'] else None,
                      param['TopLessee'][1]['percent'] /
                      100 if param['TopLessee'][1]['Top2'] else None,
...
data['NoneStatus'] = data['billing'].apply(
    lambda x: 1 if (x != 'ON' and x != 'OF') else 0)

# ONE HOT -- all lessee
for lesseeName in data['customer'].value_counts().index:
    data[lesseeName] = data['customer'].apply(
        lambda x: 1 if x == lesseeName else 0)

for i in range(numLimit):
    # Container Age
    if fleetAgeLimit[i]:
        column_name = 'FleetAge{0}'.format(i)
        data[column_name] = data['fleet_year'].apply(
            lambda x: 1 if fleetAgeLowBound[i] <= x < fleetAgeUpBound[i] else 0)
    # Weighted Age
    if weightedAgeLimit[i]:
        column_name = 'WeightedAge{0}'.format(i)
        data[column_name] = data['weighted_age'].apply(
            lambda x: 1 if weightedAgeLowBound[i] <= x < weightedAgeUpBound[i] else 0)
    # Product Type
    if productLimit[i]:
        column_name = 'ProductType{0}'.format(i)
        data[column_name] = data['product'].apply(
            lambda x: 1 if x in productType[i] else 0)
    # Contract Type
    if contractLimit[i]:
        column_name = 'ContractType{0}'.format(i)
        data[column_name] = data['contract'].apply(
            lambda x: 1 if x in contractType[i] else 0)
...
contract = []
for i in range(numLimit):
    fleetAge.append(data['FleetAge{0}'.format(
        i)].to_numpy() if fleetAgeLimit[i] else None)
    weightedAge.append(data['WeightedAge{0}'.format(
        i)].to_numpy() if weightedAgeLimit[i] else None)
    product.append(data['ProductType{0}'.format(
        i)].to_numpy() if productLimit[i] else None)
    contract.append(data['ContractType{0}'.format(
        i)].to_numpy() if contractLimit[i] else None)
...
def BuildModel(topLesseeCandidate, TopConstraints):
    ...
    var = np.array([LpVariable('container_{0}'.format(
        i), lowBound=0, cat=LpBinary) for i in range(nbv.shape[0])])
    ...
    for i in range(numLimit):
        if statusType[i]:
            ...
            if statusType[i] == 'ON':
                if statusGeq[i]:
                    prob += lpSum(var * onHireStatus *
                                  ceu) >= statusLimit[i] * ceuSelected, "OnHire{0}>".format(i)
                else:
                    prob += lpSum(var * onHireStatus *
                                  ceu) <= statusLimit[i] * ceuSelected, "OnHire{0}<".format(i)
            if statusType[i] == 'OF':
                if statusGeq[i]:
                    prob += lpSum(var * offHireStatus *
                                  ceu) >= statusLimit[i] * ceuSelected, "OffHire{0}>".format(i)
                else:
                    prob += lpSum(var * offHireStatus *
                                  ceu) <= statusLimit[i] * ceuSelected, "OffHire{0}<".format(i)
            if statusType[i] == 'None':
                if statusGeq[i]:
                    prob += lpSum(var * noneHireStatus *
                                  ceu) >= statusLimit[i] * ceuSelected, "NoneHire{0}>".format(i)
                else:
                    prob += lpSum(var * noneHireStatus *
                                  ceu) <= statusLimit[i] * ceuSelected, "NoneHire{0}<".format(i)
    # container age
    if fleetAgeAvgLimit:
        ...
        if fleetAgeAvgGeq:
            prob += lpSum(var * fleetAgeAvg) >= fleetAgeAvgLimit * \
                numSelected, "FleetAgeAvg>"
        else:
            prob += lpSum(var * fleetAgeAvg) <= fleetAgeAvgLimit * \
                numSelected, "FleetAgeAvg<"
    for i in range(numLimit):
        if fleetAgeLimit[i]:
            ...
            if fleetAgeGeq[i]:
                prob += lpSum(var * fleetAge[i]) >= fleetAgeLimit[i] * \
                    numSelected, "FleetAge{0}>".format(i)
            else:
                prob += lpSum(var * fleetAge[i]) <= fleetAgeLimit[i] * \
                    numSelected, "FleetAge{0}<".format(i)
    # weighted age
    if weightedAgeAvgLimit:
        ...
        if weightedAgeAvgGeq:
            prob += lpSum(var * weightedAgeAvg) >= weightedAgeAvgLimit * \
                ceuSelected, "WeightedAgeAvg>"
        else:
            prob += lpSum(var * weightedAgeAvg) <= weightedAgeAvgLimit * \
                ceuSelected, "WeightedAgeAvg<"
    for i in range(numLimit):
        if weightedAgeLimit[i]:
            ...
            if weightedAgeGeq[i]:
                prob += lpSum(var * weightedAge[i]) >= weightedAgeLimit[i] * \
                    ceuSelected, "WeightedAge{0}>".format(i)
            else:
                prob += lpSum(var * weightedAge[i]) <= weightedAgeLimit[i] * \
                    ceuSelected, "WeightedAge{0}<".format(i)
    # product
    for i in range(numLimit):
        if productLimit[i]:
            ...
            if productGeq[i]:
                prob += lpSum(var * product[i] * ceu) >= productLimit[i] * \
                    ceuSelected, "Product{0}>".format(i)
            else:
                prob += lpSum(var * product[i] * ceu) <= productLimit[i] * \
                    ceuSelected, "Product{0}<".format(i)
    # lessee
    for i in range(numLimit):
        if lesseeLimit[i] and lesseeType[i] in lesseeOneHot:
            ...
            if lesseeGeq[i]:
                prob += lpSum(var * lesseeOneHot[lesseeType[i]] *
                              ceu) >= lesseeLimit[i] * ceuSelected, "Lessee{0}>".format(i)
            else:
                prob += lpSum(var * lesseeOneHot[lesseeType[i]] *
                              ceu) <= lesseeLimit[i] * ceuSelected, "Lessee{0}<".format(i)
    ...
    if topLesseeLimit[0] and len(topLesseeCandidate) >= 1:
        for i in topLesseeCandidate:
            if {i} not in TopConstraints:
                ...
                prob += lpSum(var * ceu *
                              lesseeOneHot[i]) <= topLesseeLimit[0] * ceuSelected, "Top1:{0}<".format(i)
        ...
        for i in topLesseeCandidate:
            for j in topLesseeCandidate:
                if {i, j} not in TopConstraints and len({i, j}) == 2:
                    ...
                    prob += lpSum(var * ceu * np.sum(np.stack([lesseeOneHot[i], lesseeOneHot[j]]),
                                  axis=0)) <= topLesseeLimit[1] * ceuSelected, "Top2:{0}&{1}<".format(i, j)
            ...
            for j in topLesseeCandidate:
                for k in topLesseeCandidate:
                    if {i, j, k} not in TopConstraints and len({i, j, k}) == 3:
                        ...
                        prob += lpSum(var * ceu * np.sum(np.stack([lesseeOneHot[i], lesseeOneHot[j], lesseeOneHot[k]]),
                                      axis=0)) <= topLesseeLimit[2] * ceuSelected, "Top3:{0}&{1}&{2}<".format(i, j, k)
    ...
    for i in range(numLimit):
        if contractLimit[i]:
            ...
            if contractGeq[i]:
                prob += lpSum(var * contract[i] * ceu) >= contractLimit[i] * \
                    ceuSelected, "ContractType{0}>".format(i)
            else:
                prob += lpSum(var * contract[i] * ceu) <= contractLimit[i] * \
                    ceuSelected, "ContractType{0}<".format(i)
...
while True:
    ...
    top3Lessee = heapq.nlargest(3, [(lesseeName, sum(result * lesseeOneHot[lesseeName]))
                                for lesseeName in data['customer'].value_counts().index], key=lambda x: x[1])
...
if 1:
    ...
    print('======================================================================')
    print("nbv: {0} between {1} - {2}".format(round(sum(result*nbv),
          4), minTotalNbv, maxTotalNbv))
    print("cost: {0} between {1} - {2}".format(round(sum(result *
          cost), 4), minTotalCost, maxTotalCost))
    print('billing status:')
    for i in range(numLimit):
        if statusType[i]:
            if statusType[i] == 'ON':
                print('\t OnHire is {0}, -- {1}'.format(round(sum(result *
                      onHireStatus*ceu)/sum(result*ceu), 4), statusLimit[i]))
            if statusType[i] == 'OF':
                print('\t OffHire is {0}, -- {1}'.format(
                    round(sum(result*offHireStatus*ceu)/sum(result*ceu), 4), statusLimit[i]))
            if statusType[i] == 'None':
                print('\t NoneHire is {0}, -- {1}'.format(
                    round(sum(result*noneHireStatus*ceu)/sum(result*ceu), 4), statusLimit[i]))

    print("container age:")
    if fleetAgeAvgLimit:
        print('\t container average age is {0}, -- {1}'.format(
            round(sum(result*fleetAgeAvg)/sum(result), 4), fleetAgeAvgLimit))
    for i in range(numLimit):
        if fleetAgeLimit[i]:
            print("\t container age from {0} to {1} is {2}, -- {3}:".format(
                fleetAgeLowBound[i], fleetAgeUpBound[i], round(sum(result*fleetAge[i])/sum(result), 4), fleetAgeLimit[i]))

    print("weighted age:")
    if weightedAgeAvgLimit:
        print('\t weighted average age is {0}, -- {1}'.format(
            round(sum(result * weightedAgeAvg)/sum(result*ceu), 4), weightedAgeAvgLimit))
    for i in range(numLimit):
        if weightedAgeLimit[i]:
            print("\t weighted age from {0} to {1} is {2}, -- {3}:".format(weightedAgeLowBound[i], weightedAgeUpBound[i], round(
                sum(result * weightedAge[i])/sum(result*ceu), 4), weightedAgeLimit[i]))

    print("product:")
    for i in range(numLimit):
        if productLimit[i]:
            print("\t product {0} is {1}, -- {2}:".format(productType[i], round(
                sum(result*product[i]*ceu)/sum(result*ceu), 4), productLimit[i]))

    print("lessee:")
    for i in range(numLimit):
        if lesseeLimit[i]:
            print("\t lessee {0} is {1}, -- {2}:".format(lesseeType[i], round(
                sum(result*lesseeOneHot[lesseeType[i]]*ceu)/sum(result*ceu), 4), lesseeLimit[i]))

    print('Top lessee:')
    numLessee = {lesseeName: value(lpSum(
        var*lesseeOneHot[lesseeName]*ceu)) for lesseeName in data['customer'].value_counts().index}
    sortedLessee = list(numLessee.items())
    top3Lessee = heapq.nlargest(3, sortedLessee, key=lambda x: x[1])
    if topLesseeLimit[0]:
        print('\t top 1 {0} is {1}, -- {2}'.format(
            top3Lessee[0][0], top3Lessee[0][1]/sum(result*ceu), topLesseeLimit[0]))
    if topLesseeLimit[1]:
        if len(top3Lessee) >= 2:
            print('\t top 2 {0} {1} is {2}, -- {3}'.format(top3Lessee[0][0], top3Lessee[1][0], (
                top3Lessee[0][1]+top3Lessee[1][1])/sum(result*ceu), topLesseeLimit[1]))
        else:
            print('Only one lessee.')
    if topLesseeLimit[2]:
        if len(top3Lessee) >= 3:
            print('\t top 3 {0} {1} {2} is {3}, -- {4}'.format(top3Lessee[0][0], top3Lessee[1][0], top3Lessee[2][0], (
                top3Lessee[0][1]+top3Lessee[1][1]+top3Lessee[2][1])/sum(result*ceu), topLesseeLimit[2]))
        else:
    ...
    for i in range(numLimit):
        if contractLimit[i]:
            print("\t contract type {0} is {1}, -- {2}:".format(contractType[i], round(
                sum(result*contract[i]*ceu)/sum(result*ceu), 4), contractLimit[i]))
...
outputData = data[['unit_id', 'contract_num', 'cost', 'product', 'customer',
                   'contract', 'nbv', 'billing', 'fleet_year', 'weighted_age', 'ceu']].copy()
outputData.columns = ['Unit Id Fz', 'Contract Num', 'Cost', 'Product', 'Contract Cust Id',
                      'Contract Lease Type', 'Nbv', 'Billing Status Fz', 'Fleet Year Fz', 'Age x CEU', 'Ceu Fz']
